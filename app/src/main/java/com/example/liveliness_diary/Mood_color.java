...
//색상 정하기
public class Mood_color extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.mood_color);
        ...
        mood_1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (mood_arr[0] == null) {   //첫번째 배열방이 비어있다면
                    ...
                    Toast.makeText(Mood_color.this, "두 가지 색만 선택할 수 있어!", Toast.LENGTH_SHORT).show();
                }
            }
        });
...
//        if(mood_arr[0] != null && mood_arr[1] != null) {    //두 가지 색을 다 선택했을 경우
//            Mood();
//        }
    }

    public void Mood() {
        //Toast.makeText(this, mood_arr[0], Toast.LENGTH_SHORT).show();
        //Toast.makeText(this, mood_arr[1], Toast.LENGTH_SHORT).show();
        ...
        Intent intent = new Intent(Mood_color.this, Mood_grape.class);
        intent.putExtra("userId", userId);
        startActivity(intent);

        /*
        databaseReference.child("user_grape").child(userId).child("mood_grape").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                String grape = snapshot.getValue(String.class);
                if (grape.equals("grape")) {    //mood_grape가 존재하지 않는다면
                    Log.d("TAG", "PICK : " + "mood_grape 존재하지않음");
                    //포도 선택하는 곳으로 가기
                    Intent intent = new Intent(Mood_color.this, Mood_grape.class);
                    intent.putExtra("userId", userId);
                    startActivity(intent);
                } else {    //mood_grape가 존재한다면
                    Log.d("TAG", "PICK : " + "mood_grape 존재");
                    //메인화면으로 가기
                    Intent intent = new Intent(Mood_color.this, Diary.class);
                    intent.putExtra("userId", userId);
                    startActivity(intent);
                }
            }
